...
def dbt_sanctions_fr_dag():
    ...
    sanctions_fr_task = DbtTaskGroup(
        group_id="sanctions_fr",
        project_config=DBT_PROJECT_CONFIG,
        profile_config=DBT_CONFIG,
        execution_config=DBT_EXECUTION_CONFIG,
        render_config=RenderConfig(
              load_method=LoadMode.DBT_LS,
              select=['+fct_sanctions_fr'],
          )
    )

    sanctions_fr_stats_task = DbtTaskGroup(
        group_id="sanctions_stats",
        project_config=DBT_PROJECT_CONFIG,
        profile_config=DBT_CONFIG,
        execution_config=DBT_EXECUTION_CONFIG,
        render_config=RenderConfig(
              load_method=LoadMode.DBT_LS,
              select=['+fct_sanctions_stats'],
          )
    )

    sanctions_fr_task >> sanctions_fr_stats_task 
    
dbt_sanctions_fr_dag()